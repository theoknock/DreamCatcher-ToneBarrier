        ...
        if ([self startEngine])
        {
            ...
            for (int i = 0; i < 2; i++)
            {
                ...
                render_buffer[i] = ^(dispatch_queue_t __strong concurrent_queue, dispatch_queue_t __strong serial_queue, AVAudioPlayerNode * __strong player_node, struct Randomizer * __strong duration_randomizer, struct Randomizer * __strong frequency_randomizer, struct ToneDuration *__strong tone_duration) {
                    ...
                    {
                            buffer_rendered(^AVAudioPCMBuffer * (void (^buffer_sample)(AVAudioFrameCount, double, StereoChannelOutput, double, float *))
                                        {
                            ...
                            return pcmBuffer;
                        } (^(AVAudioFrameCount sample_count, double frequency, StereoChannelOutput stereo_channel_output, double gamma, float * samples) {
